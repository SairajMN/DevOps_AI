 ...
 * DevOps AI chat sidebar interface
 */
...
export class DevOpsAIChatProvider implements vscode.WebviewViewProvider {
  ...
  constructor(extensionUri: vscode.Uri) {
    ...
    this._serverUrl = vscode.workspace
  ...
  private async _handleUserMessage(message: string) {
    ...
    try {
      ...
      if (
        ...
        lowerMessage.includes("run my test")
      ) {
  ...
  private async _chat(message: string): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to get AI response. Is the server running at ${this._serverUrl}?`,
      );
    }
  }

  private async _executeTask(
    task: "test" | "build" | "install" | "analyze",
  ): Promise<TaskResult> {
    ...
    try {
      let command = "";
      const packageJson = await this._fileExists(
    ...
    try {
      ...
      if (data.analysis) {
        return {
          success: true,
          output:
            `**Error Type:** ${data.analysis.error_type}\n\n` +
    ...
    try {
      ...
      if (data.fix) {
        return {
          success: true,
          output:
            `**Fix Description:** ${data.fix.fix_description}\n\n` +
