...
class TaskExecutor {
    ...
    async executeCommand(command) {
        console.log(`${colors.yellow}⚡ Executing: ${colors.cyan}${command}${colors.reset}\n`);
        try {
    ...
    async analyzeLogs(logPath) {
        const logFile = logPath || path.join(this.projectContext.path, "logs", "error.log");
...
class ClineLikeCLI {
    ...
    async start(projectPath) {
        ...
        if (this.projectContext) {
            ...
            console.log(`${colors.green}📁 Project: ${colors.bold}${this.projectContext.name}${colors.reset}`);
            console.log(`${colors.blue}   Type: ${this.projectContext.type} | Package Manager: ${this.projectContext.packageManager}${colors.reset}`);
            console.log(`${colors.dim}   Path: ${this.projectContext.path}${colors.reset}\n`);
        }
        printDivider();
        console.log(`${colors.cyan}💬 Chat with your DevOps AI assistant. Type ${colors.bold}help${colors.reset}${colors.cyan} for commands.${colors.reset}\n`);
        this.chatLoop();
    }
    chatLoop() {
        this.rl.question(`${colors.green}You:${colors.reset} `, async (input) => {
            ...
            // Regular chat
            printThinking();
            const response = await this.aiClient.chat(trimmed, this.projectContext || undefined);
            clearThinking();
    ...
    async handleActionCommand(input) {
        if (!this.taskExecutor) {
            console.log(`${colors.red}❌ No project context available. Open a project first.${colors.reset}\n`);
            return;
        }
        const lower = input.toLowerCase();
        if (lower.includes("test")) {
            ...
            printThinking();
            const response = await this.aiClient.chat(`Help me fix this issue: ${input}`, this.projectContext || undefined);
            clearThinking();
            console.log(`\n${colors.magenta}Assistant:${colors.reset} ${response}\n`);
            printDivider();
        }
            ...
            printThinking();
            const response = await this.aiClient.chat(input, this.projectContext || undefined);
            clearThinking();
            console.log(`\n${colors.magenta}Assistant:${colors.reset} ${response}\n`);
            printDivider();
        }
    ...
    async handleCommand(command) {
        ...
        switch (cmd) {
            case "open":
                if (args[0]) {
                    ...
                    if (fs.existsSync(projectPath)) {
                        this.projectContext = await detectProject(projectPath);
                        this.taskExecutor = new TaskExecutor(this.projectContext);
                        console.log(`${colors.green}✅ Opened project: ${this.projectContext.name}${colors.reset}\n`);
                    }
                    else {
                        console.log(`${colors.red}❌ Path not found: ${projectPath}${colors.reset}\n`);
                    }
                }
                else {
                    console.log(`${colors.yellow}Usage: /open <project-path>${colors.reset}\n`);
                }
                break;
            case "test":
                if (this.taskExecutor) {
            ...
            case "help":
                ...
                break;
            default:
                console.log(`${colors.yellow}Unknown command. Type ${colors.bold}help${colors.reset}${colors.yellow} for available commands.${colors.reset}\n`);
        }
    }
    showHelp() {
        console.log(`
